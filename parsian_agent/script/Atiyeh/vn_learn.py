...
vn_pub = matrix_out[:,1:2]
######################## network layers
l1 = tf.layers.dense(v_in, 15, tf.nn.sigmoid) # check out
l2 = tf.layers.dense(l1, 15, tf.nn.sigmoid)
...
for step in range(50000):
    index = np.random.randint(0, vn_real.shape[0], BATCH_SIZE)
    _, l ,pred = sess.run([train_op, loss, v_pred],{v_in: v_real[index,:] , v_out: vn_pub[index,:]})
    if step % 10 == 0:
...
np.savetxt('pred_vn.txt' , pred_test , fmt = '%3.8f', delimiter= ' ')
pred_test = np.loadtxt("pred_vn.txt")
...
v_test = v_real
pred_test = sess.run(v_pred,{v_in : v_test})
np.savetxt('pred1.txt' , pred_test , fmt = '%3.8f', delimiter= ' ')
pred_test = np.loadtxt("pred1.txt")
plt.plot(vn_pub[2500:8500])
plt.plot(pred_test[2500:8500])
plt.show()