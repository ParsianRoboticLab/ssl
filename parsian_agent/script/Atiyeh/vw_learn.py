...
for step in range(50000):
    index = np.random.randint(0, vw_real.shape[0], BATCH_SIZE)
    _, l ,pred = sess.run([train_op, loss, v_pred],{v_in: v_real[index,:] , v_out: vw_pub[index,:]})
    if step % 10 == 0:
...
np.savetxt('pred.txt' , pred_test , fmt = '%3.8f', delimiter= ' ')
pred_test = np.loadtxt("pred.txt")
plt.plot(vw_pub[0:6000])
plt.plot(pred_test[0:6000])
plt.show()